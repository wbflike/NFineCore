...
$(function ($) {
    ...
    $(window).load(function () {
        ...
        //    id: "Form",
        //    title: "新增用户",
        //    url: "/WeixinManage/WxApp/ChoiceWxApp",
        //    width: "700px",
        //    height: "510px",
...
function GetLoadNav() {
    ...
    $.each(data, function (i) {
        var row = data[i];
        if (row.ParentId == "0" && row.IsDisplay == false) {
            _menuhtml = '<ul  onclick="navbar_click(this)"  class="nav navbar-nav pull-left"><li> <a style="font-size:13px;" id="' + row.Id + '" class="btn"   data-id="' + row.Id + '"><i class="' + row.Icon + '"></i>&ensp;' + row.FullName + '</a>  </li> </ul>';
            ...
            //if (i == 0) {
                ...
                if (childNodes.length > 0) {
                    $.each(childNodes, function (i) {
                        ...
                        if (subrow.ParentId == row.Id && subrow.IsDisplay == false) {
                            //二级菜单
                            $("#menu_title").text(row.FullName);
                            $("#" + row.Id + "").css("background-color", "#14A689");
                            _html += '<li>';
                            ...
                            if (subchildNodes.length > 0) {
                                ...
                                $.each(subchildNodes, function (i) {                                    
                                    ...
                                    if (subrowsj.ParentId == subrow.Id && subrowsj.IsDisplay == false) {
                                        _html += '<li>';
                                        _html += '<a class="menuItem" data-id="' + subrowsj.Id + '" href="' + subrowsj.UrlAddress + '" data-index="' + subrowsj.SortCode + '" data-target="' + subrowsj.Target + '" title="' + subrowsj.FullName + '">' + subrowsj.FullName + '</a>';
                                        _html += '</li>';
                                        // alert(subrowsj.FullName);
                                    }
                                });
                                _html += '</ul>';
                            } else {
                                _html += '<a  class="menuItem"  data-id="' + subrow.Id + '" href="' + subrow.UrlAddress + '" data-index="' + subrow.SortCode + '" data-target="' + subrow.Target + '"><i class="' + subrow.Icon + '"></i><span>' + subrow.FullName + '</span></a>';
                            }
                            _html += '</li>';
                        }
                    });
                    ...
                    if (subrow.ParentId == id) {
                        ...
                        if (subchildNodes.length > 0) {
                            ...
                            $.each(subchildNodes, function (i) {
                                var subrowsj = subchildNodes[i];
                                //三级菜单
                                if (subrowsj.ParentId == subrow.Id) {
                                    _html += '<li>';
                                    _html += '<a class="menuItem" data-id="' + subrowsj.Id + '" href="' + subrowsj.UrlAddress + '" data-index="' + subrowsj.SortCode + '">' + subrowsj.FullName + '</a>';
                                    _html += '</li>';
                                }
                            });
                            _html += '</ul>';
                        } else {
                            _html += '<a  class="menuItem"  data-id="' + subrow.Id + '" href="' + subrow.UrlAddress + '" data-index="' + subrow.SortCode + '"><i class="' + subrow.Icon + '"></i><span>' + subrow.FullName + '</span></a>';
                        }
                        _html += '</li>';
                    }
                });
